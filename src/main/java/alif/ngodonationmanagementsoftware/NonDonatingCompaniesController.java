...
public class NonDonatingCompaniesController {
    ...
    private static final String NON_DONATING_FILE = "non_donating_companies.bin";
    ...
    public void showList(ActionEvent event) {
        ...
        for (NewCompany c : getAllCompanies()) {
            if (c.getDonationAmount() == 0.0) { // Only non-donating
    ...
    public void downloadList(ActionEvent event) {
        ...
        chooser.setInitialFileName("NonDonatingCompanies_" +
    ...
    private void saveListToFile(ObservableList<NonDonatingCompany> list) {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(NON_DONATING_FILE))) {
